...
ANCHORS         = (np.array(YOLO_ANCHORS).T/STRIDES).T  ##the adjusted anchor boxes
...
class BatchNormalization(BatchNormalization):
    ...
    def call(self, x, training=False):
        ...
        training = tf.logical_and(training, self.trainable) ##ensures that the layer is only updated during training when it is set to be trainable
        ##self.trainable is a boolean attribute of the BatchNormalization layer
        ##that indicates whether the layer's weights should be updated during training or not
...
def convolutional(input_layer, filters_shape, downsample=False, activate=True, bn=True):
    if downsample:
        ...
        padding = 'valid'
        strides = 2
        ##the padding is necessary to avoid losing information on the edges of the input during downsampling
    else:
        strides = 1
        padding = 'same'
